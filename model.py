...
class SentenceVAE(nn.Module):
    ...
    def forward(self, input_sequence, length):
        ...
        logp = nn.functional.log_softmax(self.outputs2vocab(padded_outputs.view(-1, padded_outputs.size(2))))
    ...
    def inference(self, n=4, z=None):
        ...
        generations = self.tensor(batch_size, self.max_sequence_length+1).fill_(self.pad_idx).long()

        t=0
        while(t<self.max_sequence_length+1 and len(running_seqs)>0):
...
class SentenceIAF(nn.Module):

    def __init__(self, vocab_size, embedding_size, rnn_type, hidden_size, word_dropout, latent_size,
                sos_idx, eos_idx, pad_idx, max_sequence_length, num_layers=1, bidirectional=False):

        super(SentenceIAF,self).__init__()
        self.tensor = torch.cuda.FloatTensor if torch.cuda.is_available() else torch.Tensor
    ...
    def forward(self, input_sequence, length):
        ...
        batch_size, max_len = input_sequence.size()
        sorted_lengths, sorted_idx = torch.sort(length, descending=True)
        input_sequence = input_sequence[sorted_idx]
        ...
        for t in range(0,max_len):
            mean_t,std_t = self.iaf(z_prev, hidden)
            std_t = nn.Sigmoid()(std_t)
            z_new = std_t * z + (1-std_t) * mean_t
#             z_new = z_curr * std_t + mean_t
            z_prev = z_new;
